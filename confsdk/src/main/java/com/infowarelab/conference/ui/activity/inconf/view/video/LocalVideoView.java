 ...
 */
public class LocalVideoView extends SurfaceView implements PreviewCallback,
        Callback {
    ...
    public void startCamera() {
        ...
        if (mHolder != null && mSurfaceCreated) {
        ...
        //setBackgroundColor(R.color.app_video_background);

        Log.d(TAG, "LocalVideoView.startCamera");
    }
...
//			Toast.makeText(activity, activity.getResources().getString(R.string.nocamerapermission), Toast.LENGTH_LONG).show();
    ...
    private void setCameraParameters(int degrees) {
        ...
        if (initVideo) {
            if (isPortrait) {
                Log.d("InfowareLab.Encoder", "LocalVideoView.initializeMyVideo: " + cameraHeight + "x" + cameraWidth);
                videoCommon.initializeMyVideo(cameraHeight, cameraWidth, FRAME_RATE);
            }
            else {
                Log.d("InfowareLab.Encoder", "LocalVideoView.initializeMyVideo: " + cameraWidth + "x" + cameraHeight);
        ...
        if (isShareing && ConferenceApplication.isVideoMeeting) {
            ...
            if (isPortrait) {//竖屏
                ...
                } else  //software encode
                {
                    ...
                    // Log.d(TAG,"landscape data length = "+data.length);
                }
            }
        }
